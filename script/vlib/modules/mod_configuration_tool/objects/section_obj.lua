...
local mct_section = {
    ...
    ---@type UIComponent UI Object for the header row itself.
    _header = nil,
...
local __mct_section_mt = {
    __index = mct_section,
    __tostring = function(self) return "MCT_SECTION [" .. self._key .. "]" end,
}

--- For internal use only. Use @{mct_mod:add_new_section}.
---@param key string The key to identify the new mct_section.
---@param mod mct_mod The mct_mod this section is a member of.
function mct_section.new(key, mod)
    local o = table.copy_add({
        _key = key,
        _mod = mod
    }, mct_section)
    setmetatable(o, __mct_section_mt)
...
function mct_section:uic_visibility_change(event_free)
    ...
    for i = 1, #attached_rows do
        if not is_uicomponent(attached_rows[i]) then
            ...
            _SetVisible(row, visibility)
        end
    end

    -- also change the state of the UI header
    if visibility then
        _SetState(self._header, "selected")
        -- set to selected
    else
        _SetState(self._header, "active")
        -- set to active
    end
...
function mct_section:get_tooltip_text()
    -- default to checking the loc files
    local text = effect.get_localised_string("mct_"..self:get_mod():get_key().."_"..self:get_key().."_section_tooltip_text")
...
---@param option_obj mct_option The option object to assign into this section.
