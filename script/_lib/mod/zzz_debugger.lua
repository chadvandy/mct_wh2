...
function OWR_ERROR_FINDER()
    ...
    --v function(func: function) --> any
        function safeCall(func)
            ...
            if not status then
                mct:log(tostring(result), "ERROR CHECKER")
                mct:log(debug.traceback(), "ERROR CHECKER");
            end
        ...
        function tryRequire(fileName)
            ...
            if not loaded_file then
                mct:log("Failed to find mod file with name " .. fileName)
            else
                mct:log("Found mod file with name " .. fileName)
                mct:log("Load start")
            ...
            for name,f in pairs(getmetatable(object)) do
                ...
                if name == "__index" and not is_function(f) then
                    for indexname,indexf in pairs(f) do
                        mct:log("Found in index " .. indexname);
