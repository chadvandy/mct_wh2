...
function ui_obj:trigger_popup(key, text, two_buttons, button_one_callback, button_two_callback)
    ...
    local function do_stuff()
        ...
        core:add_listener(
            ...
            function(context)
                mct:log("button pres'd")
                local button = UIComponent(context.component)
                mct:log("button pres'd 1")
                local id = context.string
                mct:log("button pres'd 2")
...
function ui_obj:open_frame()
    ...
    if not is_uicomponent(test) then
        ...
        local left_panel = self.left_panel_holder
        local filter_holder = find_uicomponent(left_panel, "filter_holder")
        local filter_text_input = find_uicomponent(filter_holder, "mct_filter_text_input")
        
        if is_uicomponent(filter_text_input) then
            local filter_x = find_uicomponent(filter_text_input, "mct_filter_x")

            if is_string(self.filter_text) and self.filter_text ~= "" then
                filter_text_input:SetStateText(self.filter_text)
                self:apply_filter(self.filter_text)
                filter_x:SetState("active")
            else
                filter_text_input:SetStateText("")
                self:apply_filter("")
                filter_x:SetState("inactive")
            end
        end
        -- TODO setup
...
function ui_obj:create_panels()
    ...
    local filter_text_input = core:get_or_create_component("mct_filter_text_input", "ui/mct/text_box_left", filter_holder)
    ...
    local filter_x = core:get_or_create_component("mct_filter_x", "ui/templates/square_medium_button", filter_text_input)
    filter_x:SetDockingPoint(6)
    filter_x:SetDockOffset(-5, 0)
    filter_x:Resize(filter_button:Width(), filter_button:Height())
    filter_x:SetState("inactive")
...
function ui_obj:do_log_list_view()
    ...
    for line_num, line_txt in pairs(lines) do
        ...
        self:SetStateText(text_component, str)
    ...
    local function do_stuff()
        ...
        for mod_key, mod_data in pairs(changed_settings) do
            if selected_mod and mod_key ~= selected_mod then
                ...
                local mod_display = UIComponent(list_box:CreateComponent(mod_key, "ui/vandy_lib/text/la_gioconda/unaligned"))
                ...
                for option_key, option_data in pairs(mod_data) do
                    ...
                    local option_display = UIComponent(option_row:CreateComponent(option_key.."_display", "ui/vandy_lib/text/la_gioconda/unaligned"))
                    ...
                    local old_value_uic = core:get_or_create_component("old_value", "ui/vandy_lib/text/la_gioconda/unaligned", option_row)
                    ...
                    local new_value_uic = core:get_or_create_component("new_value", "ui/vandy_lib/text/la_gioconda/unaligned", option_row)
...
function ui_obj:set_filter_text_in_ui(str)
    if not is_string(str) then
        -- errmsg
        return false
    end
    ...
    local filter_text_input = find_uicomponent(left_panel_holder, "filter_holder", "mct_filter_text_input")
    ...
    filter_text_input:SetStateText(str)
...
function ui_obj:set_filter_active()
    ...
    core:add_listener(
        ...
        "ComponentLClickUp",
        function(context)
            return context.string ~= text_input_key and context.string ~= "mct_filter_x"
        end,
        function(context)
    ...
    local function apply()
        ...
        self.filter_text = uic:GetStateText()
...
core:add_listener(
    ...
    "ComponentLClickUp",
    function(context)
        return context.string == "mct_filter_x"
    end,
    function(context)
        mct:log("mct filter x pressed")
        ui_obj.filter_text = ""
        ui_obj:set_filter_text_in_ui("")
        ui_obj:apply_filter("")
        mct:log("mct filter x end")
    end,
    true
)
