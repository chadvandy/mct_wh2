    ...
    function(context)
        ...
        if is_uicomponent(mod_settings_panel) then
            ...
            if is_uicomponent(finalize_button) then
                ...
                ui_obj:uic_SetTooltipText(finalize_button, "Cannot change settings while in battle!", true)

                local finalize_button_txt = find_uicomponent(finalize_button, "button_txt")
                ui_obj:uic_SetStateText(finalize_button_txt, "[[col:red]]Finalize Settings[[/col]]")
            end
        end
    end,
    true
